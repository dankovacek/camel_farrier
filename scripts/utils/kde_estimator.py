...
def measurement_error_bandwidth_function(x: jnp.ndarray) -> jnp.ndarray:
    error_points = jnp.array([1e-4, 1e-3, 1e-2, 1e-1, 1.0, 1e1, 1e2, 1e3, 1e4, 1e5])
...
def adaptive_bandwidths(uar: jnp.ndarray, da: float) -> jnp.ndarray:
    ...
    if len(unique_UAR) < 2:
        print(
            f"    not enough unique values in runoff data ({len(unique_UAR)}), adding noise to the data according to the measurement error model."
        )
    ...
    log_midpoints = jnp.concatenate(
        (jnp.array([left_mirror]), log_midpoints, jnp.array([right_mirror]))
    )
...
def kde_kernel(log_data, bw_values, log_grid):
    ...
    K = jnp.exp(-0.5 * U**2) / (
        H * jnp.sqrt(2 * jnp.pi)
    )  # Gaussian kernel with variable bandwidth H
