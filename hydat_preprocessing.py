...
for i, row in station_df.iloc[random_idxs].iterrows():
    ...
    # retrieve the catchment geometries and create individual files
    station_gdf = retrieve_station_geometries(
        station_id, stn_output_folder, CATCHMENT_POLYGON_DIR
    )
    # filter the water license information per catchment polygon
    licenses = gpd.sjoin(
        water_license_df,
        station_gdf["catchment"],
        predicate="within",
    )
    licenses = licenses[water_license_df.columns]
    # if any licenses, save the points
    if not licenses.empty:
        licenses.to_file(
            stn_output_folder / f"{station_id}_water_licenses.geojson", driver="GeoJSON"
        )
